...
export function PortfolioComponent() {
  ...
  return (
    <div className="min-h-screen bg-light dark:bg-dark text-dark dark:text-light transition-colors duration-300">
      ...
      <header className="fixed w-full bg-light-secondary dark:bg-dark-secondary bg-opacity-90 shadow-md z-10 transition-colors duration-300">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button onClick={toggleTheme} className="p-2 rounded-full bg-light dark:bg-dark text-dark dark:text-light">
              {isDarkMode ? '🌞' : '🌙'}
            </button>
        ...
        {isMenuOpen && (
          <nav className="md:hidden bg-light-secondary dark:bg-dark-secondary p-4 transition-colors duration-300">
            ...
            <a href="#skills" className="block py-2 hover:text-primary">Skills</a>
            <a href="#projects" className="block py-2 hover:text-primary">Projects</a>
      ...
      {/* Hero Section */}
      <section id="home" className="relative min-h-screen flex items-center justify-center bg-light dark:bg-dark transition-colors duration-300 pt-20">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-5xl md:text-7xl font-bold mb-4 text-primary">John Doe</h2>
          <p className="text-xl md:text-3xl mb-8 text-dark dark:text-light">Full Stack Developer</p>
          ...
          <a href="#contact" className="inline-block bg-primary text-white font-bold py-3 px-6 rounded-full hover:bg-primary-dark transition duration-300">
            Get in Touch
          </a>
        </div>
      </section>
      ...
      {/* Skills Section */}
      <section id="skills" className="py-16 bg-light-secondary dark:bg-dark-secondary px-4 transition-colors duration-300 pt-24">
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['JavaScript', 'React', 'Node.js', 'Python', 'SQL', 'Git', 'AWS', 'Docker'].map((skill) => (
              <div key={skill} className="bg-light dark:bg-dark rounded-lg p-4 text-center transition-colors duration-300">
                {skill}
              </div>
            ))}
          </div>
        ...
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {projects.map((project, index) => (
              <div key={index} className="bg-light-secondary dark:bg-dark-secondary rounded-lg overflow-hidden shadow-lg transition-colors duration-300">
                <img src={project.image} alt={project.title} className="w-full h-48 object-cover" />
                <div className="p-6">
                  <h4 className="text-xl font-bold mb-2">{project.title}</h4>
                  <p className="text-dark dark:text-light opacity-80 mb-4">{project.description}</p>
                  <div className="flex justify-between">
                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">GitHub</a>
                    <a href={project.demo} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Live Demo</a>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Contact Section */}
      <section id="contact" className="py-16 bg-light-secondary dark:bg-dark-secondary px-4 transition-colors duration-300 pt-24">
